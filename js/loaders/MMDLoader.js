...
THREE.MMDLoader = ( function () {
	...
	MMDLoader.prototype = {
		...
		loadAnimation: function ( url, object, onLoad, onProgress, onError ) {
			...
			this.loadVMD( url, function ( vmd ) {
				...
				that.onBeforeBuildAnimation && that.onBeforeBuildAnimation( url, vmd );
		...
		loadPMD: function ( url, onLoad, onProgress, onError ) {
			...
			this.loader
				...
				.load( url, function ( buffer ) {
					...
					//var pmd = parser.parsePmd( buffer, true )
					//console.log(pmd)
					// pmd.rigidBodies[36].boneIndex = 74;
					// pmd.rigidBodies[36].weight = 0;
					// pmd.rigidBodies[36].type = 0;

					// pmd.rigidBodies[40].boneIndex = 74;
					// pmd.rigidBodies[40].weight = 0;
					// pmd.rigidBodies[40].type = 0;

					// pmd.rigidBodies[37].boneIndex = 42;
					// pmd.rigidBodies[37].weight = 0;
					// pmd.rigidBodies[37].type = 0;

					// pmd.rigidBodies[41].boneIndex = 42;
					// pmd.rigidBodies[41].weight = 0;
					// pmd.rigidBodies[41].type = 0;
	...
	GeometryBuilder.prototype = {
		...
		build: function ( data ) {
			...
			function updatePosition( morph, ratio , override) {
				...
				var existIndex = morphPositions.findIndex( x => x.name == morph.name);
			...
			function updateUV( morph, ratio , override) {

				var attribute = new THREE.Float32BufferAttribute( data.metadata.vertexCount * 2, 2 );
				attribute.name = morph.name;

				for ( var j = 0; j < data.metadata.vertexCount * 2; j ++ ) {

					attribute.array[ j ] = uvs[ j ];
				...
				for ( var i = 0; i < morph.elementCount; i ++ ) {
					...
					attribute.array[ index * 2 + 0 ] += element.uv[ 0 ] * ratio;
					attribute.array[ index * 2 + 1 ] += element.uv[ 1 ] * ratio;

				}

				var existIndex = morphUVs.findIndex( x => x.name == morph.name);

				if( existIndex === -1 ){
					morphUVs.push( attribute );
				}else if( override ){
					morphUVs[existIndex] = attribute;
				}	

			}

			function updateAttributes( morph, ratio, itemSize, rawData, attrName ){
				var attribute = new THREE.Float32BufferAttribute( data.metadata.vertexCount * itemSize, itemSize );
				attribute.name = morph.name;

				for ( var j = 0; j < data.metadata.vertexCount * itemSize; j ++ ) {

					attribute.array[ j ] = rawData[ j ];
				...
				for ( var i = 0; i < morph.elementCount; i ++ ) {
					...
					for( var i = 0; i < itemSize; i++ ){
						attribute.array[ index * itemSize + i ] += element[attrName][ i ] * ratio;
			...
			for ( var i = 0; i < data.metadata.morphCount; i ++ ) {
				...
				if ( data.metadata.format === 'pmd' ) {
					...
					} else if ( morphPositions.find( x => x.name == morph.name ) ) {

						console.warn(`skip morph because group,  ${morph.name}`);
			...
			geometry.morphAttributes.uv = morphUVs;

			geometry.backupAttributes = geometry.backupAttributes || {};
			geometry.backupAttributes.uv = uvs.slice();
	...
	MaterialBuilder.prototype = {
		...
		build: function ( data, geometry, onProgress, onError ) {
			...
			for ( var i = 0; i < data.metadata.materialCount; i ++ ) {
				...
				var material = new THREE.MeshToonMaterial( params );

				if ( ! params.transparent && params.map !== undefined) {

					this._checkImageTransparency( params.map, geometry, i , function(transparent){
		...
		_checkImageTransparency: function ( map, geometry, groupIndex, callback ) {
			...
			map.readyCallbacks.push( function ( texture ) {
				...
				function simpleDetectImageTransparency( image ) {
				...
				if ( simpleDetectImageTransparency(imageData) ){

					map.transparent = true;
				}

				callback(map.transparent)

				// console.warn(geometry.morphAttributes.uv)

				// for ( var i = 0, len = geometry.morphAttributes.uv.lengh; i < len ; i++ ){

				// 	if ( detectImageTransparency(
				// 		imageData,
				// 		geometry.morphAttributes.uv[i].array,
				// 		geometry.index.array.slice( group.start, group.start + group.count ) ) ) {
						
				// 		console.warn('uv 变换透明');
				// 		console.warn(map);
				// 		map.transparent = true;
	...
	AnimationBuilder.prototype = {
		...
		buildMorphAnimation: function ( vmd, mesh ) {
			...
			//console.warn(morphTargetDictionary);

			function collectMorph( morph, morphData, override ){

	
				morphs[ morphData.name ] = morphs[ morphData.name ] || { type: morphData.type, array: [] };

				var existMorphFrame = morphs[ morphData.name ].array.find( x => x.frameNum === morph.frameNum );

				if ( existMorphFrame === undefined ) {

					morphs[ morphData.name ].array.push( morph );
					
				} else if ( override ) {

					existMorphFrame.weight = morph.weight;
				}
			...
			for ( var i = 0; i < vmd.metadata.morphCount; i ++ ) {
				...
				var morphData = mesh.geometry.userData.MMD.morphs.find( x => x.name == morphName );

				if ( morphData === undefined ) continue;
			...
			for ( var key in morphs ) {
				...
				if ( mesh.geometry.userData.MMD.format === 'pmd'  || type === 1 ){
					...
					mesh.morphWeights[key] = 0;

					tracks.push( new THREE.NumberKeyframeTrack( `.morphWeights[${key}]`, times, values ) );
				}
		...
		buildCameraAnimation: function ( vmd ) {
			...
			//console.log(times, centers, quaternions, positions, fovs)
			var tracks = [];
			...
			tracks.push( this._createTrack( 'target.distance', THREE.NumberKeyframeTrack, times, distances, pInterpolations ) );
			tracks.push( this._createTrack( 'target.rotsX', THREE.NumberKeyframeTrack, times, rotsX, qInterpolations ) );
			tracks.push( this._createTrack( 'target.rotsY', THREE.NumberKeyframeTrack, times, rotsY, qInterpolations ) );
			tracks.push( this._createTrack( 'target.rotsZ', THREE.NumberKeyframeTrack, times, rotsZ, qInterpolations ) );
