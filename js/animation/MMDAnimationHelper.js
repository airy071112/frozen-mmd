...
THREE.MMDAnimationHelper = ( function () {
	...
	function MMDAnimationHelper( params ) {
		...
		this.cameraTarget.distance = 0;
		this.cameraTarget.rotsX = 0;
		this.cameraTarget.rotsY = 0;
		this.cameraTarget.rotsZ = 0;
		...
		this.configuration = {
			...
			loopMode: params.loopMode || THREE.LoopRepeat,
			loopRepetitions: params.loopRepetitions || Infinity
		};
	...
	MMDAnimationHelper.prototype = {
		...
		enable: function ( key, enabled ) {
			...
			if ( key === 'physics' ) {
				...
				// /enabled && (this.resetPhysics = true);
		...
		_setupMeshAnimation: function ( mesh, animation ) {
			...
			if ( animation !== undefined ) {
				...
				var loopMode = this.configuration.loopMode;
				var loopRepetitions = this.configuration.loopRepetitions;

				for ( var i = 0, il = animations.length; i < il; i ++ ) {

					objects.mixer.clipAction( animations[ i ] ).setLoop( loopMode, loopRepetitions ).play();

				}

				//objects.mixer.update(0);
		...
		_setupCameraAnimation: function ( camera, animation, params ) {
			...
			objects.position = new THREE.Vector3();
		...
		_updateMeshAttributes: function (mesh) {

			if( mesh.geometry.morphAttributes.uv ){

				//mesh.geometry.attributes.uvbackup = mesh.geometry.attributes.uv.clone
				var needsUpdate = false;
				var backupArray = mesh.geometry.backupAttributes.uv;
				var uvAttributeArray = mesh.geometry.attributes.uv.array;

				for ( var j = 0, jl = backupArray.length; j < jl; j++ ){
					
					var currentValue = backupArray[ j ];

					for ( var i = 0, l = mesh.geometry.morphAttributes.uv.length; i < l ; i++ ) {

						var morphUV  = mesh.geometry.morphAttributes.uv [ i ];

						var morphArray = morphUV.array;
						var weight = mesh.morphWeights[morphUV.name] || 0;

						currentValue += (morphArray[j] - backupArray[ j ]) * weight;
					}

					if( uvAttributeArray[j] != currentValue ){
						uvAttributeArray[j] = currentValue;
						needsUpdate = true;
					}
				}

				mesh.geometry.attributes.uv.needsUpdate = needsUpdate;
			}
		...
		_animateMesh: function ( mesh, delta ) {
			...
			if ( mixer && (this.enabled.animation) ) {
				...
				this._updateMeshAttributes( mesh ); 
		...
		_animateCamera: function ( camera, delta ) {
			...
			var position = this.objects.get( camera ).position;
			...
			if ( mixer && this.enabled.cameraAnimation ) {
				...
				this.cameraTarget.rotateOnWorldAxis(new THREE.Vector3(1, 0, 0), this.cameraTarget.rotsX);
				this.cameraTarget.rotateOnWorldAxis(new THREE.Vector3(0, 1, 0), this.cameraTarget.rotsY);
				this.cameraTarget.rotateOnWorldAxis(new THREE.Vector3(0, 0, 1), this.cameraTarget.rotsZ);

				position.set( 0, 0, this.cameraTarget.distance );
				
				position.applyQuaternion(this.cameraTarget.quaternion);

				position.add(this.cameraTarget.position);

				camera.position.copy(position);
				camera.quaternion.copy(this.cameraTarget.quaternion);
		...
		_saveBones: function ( mesh ) {
			...
			for ( var i = 0, il = bones.length; i < il; i ++ ) {
				...
				var bone = bones[ i ];
				//var boneType = mesh.geometry.bones[i].rigidBodyType;
				//if( boneType == -1 ){
	...
	function AudioManager( audio, params ) {
		...
		this.playTimes = params.loopMode === THREE.LoopOnce? 1:(params.loopRepetitions || Infinity);
	...
	AudioManager.prototype = {
		...
		_shouldStartAudio: function () {
			...
			if ( this.playTimes < 1 ) return false;
