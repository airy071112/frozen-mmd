...
THREE.MMDAnimationHelper = ( function () {
	...
	MMDAnimationHelper.prototype = {
		...
		_updateMeshAttributes: function (mesh) {
			...
			if( mesh.geometry.morphAttributes.uv ){
				...
				for ( var i = 0, l = mesh.geometry.morphAttributes.uv.length; i < l ; i++ ) {
					...
					var weight = mesh.morphWeights[morphUV.name];
					...
					for ( var j = 0, jl = uvAttributeArray.length; j < jl; j++){

						var currentValue = ( 1 - weight ) * backupArray[j] + weight * morphArray[j];

						if( uvAttributeArray[j] != currentValue ){
							uvAttributeArray[j] = ( 1 - weight ) * backupArray[j] + weight * morphArray[j];
							needsUpdate = true;
						}
					}
				}
		...
		_animateMesh: function ( mesh, delta, force ) {
			...
			if ( mixer && (this.enabled.animation || force) ) {
				//mesh.pose();
				//this._updatePropertyMixersBuffer( mesh );
